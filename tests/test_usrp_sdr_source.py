...
import thread.SDRThread as sdr_thread
...
class UsrpSdrSourceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.saved_runtime = {
            "SAMPLE_RATE": sdr_thread.SAMPLE_RATE,
            "CENTER_FREQ": sdr_thread.CENTER_FREQ,
            "GAIN": sdr_thread.GAIN,
            "BANDWIDTH": sdr_thread.BANDWIDTH,
        }

    def tearDown(self):
        for key, value in self.saved_config.items():
            setattr(config, key, value)
        for key, value in self.saved_runtime.items():
            setattr(sdr_thread, key, value)

    def test_complex64_to_sc16_q11_bytes_interleaves_iq(self):
        samples = np.asarray([1.0 + 0.5j, -1.0 - 0.5j], dtype=np.complex64)

        raw = sdr_thread._complex64_to_sc16_q11_bytes(samples)
    ...
    def test_usrp_source_configures_stream_and_returns_legacy_shape(self):
        config.SDR_USRP_ARGS = "addr=192.168.5.111"
        config.SDR_USRP_CHANNEL = 1
        config.SDR_USRP_ANTENNA = "RX2"
        config.SDR_USRP_STREAM_ARGS = "spp=200"
        config.SDR_USRP_RECV_TIMEOUT = 0.5
        config.SDR_USRP_SET_BANDWIDTH = True
        sdr_thread.SAMPLE_RATE = 1_000_000.0
        sdr_thread.CENTER_FREQ = 1_575_420_000.0
        sdr_thread.GAIN = 30.0
        sdr_thread.BANDWIDTH = 500_000.0

        source = sdr_thread.UhdUsrpReceiverSource(uhd_module=FakeUhdModule)
    ...
    def test_usrp_source_skips_bandwidth_by_default(self):
        config.SDR_USRP_ARGS = "addr=192.168.5.111"
        config.SDR_USRP_CHANNEL = 0
        config.SDR_USRP_ANTENNA = ""
        config.SDR_USRP_STREAM_ARGS = ""
        config.SDR_USRP_RECV_TIMEOUT = 0.5
        config.SDR_USRP_SET_BANDWIDTH = False
        sdr_thread.SAMPLE_RATE = 5_000_000.0
        sdr_thread.CENTER_FREQ = 1_575_420_000.0
        sdr_thread.GAIN = 30.0
        sdr_thread.BANDWIDTH = 2_500_000.0
