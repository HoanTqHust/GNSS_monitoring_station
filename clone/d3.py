#!/usr/bin/env python3
"""
Standalone D3 batch detector for GNSS spoofing experiments.

This script reads UBX logs directly, builds the requested DD measurement family,
and evaluates the D3 detector without importing the shared comparison pipeline.
"""
...
MEASUREMENT_ATTRS = {
    "raw_code": "raw_code_dd_m",
    "carrier_frac": "carrier_frac_dd_cycles",
    "smoothed_code": "smoothed_code_dd_m",
}
...
def validate_config(config: D3Config) -> None:
    if config.measurement not in MEASUREMENT_ATTRS:
        raise ValueError(f"Unsupported measurement: {config.measurement}")
    ...
    if not 0.0 <= config.clean_quantile <= 1.0:
        raise ValueError("clean_quantile must be in [0, 1].")
...
def build_epoch_records(
    ...
    for epoch_index, (epoch1, epoch2) in enumerate(epoch_pairs):
        ...
        smooth_ref_sd = None
        if reference_svid in states1 and reference_svid in states2:
        ...
        for svid in candidate_svids:
            ...
            carrier_frac_dd_cycles[svid] = fractional_cycles(
                (obs2.cp_cycles - obs1.cp_cycles) - carr_ref_sd
            )
...
def iter_pairwise_abs_diffs(records: list[EpochRecord], measurement_attr: str) -> list[float]:
    diffs: list[float] = []
    for record in records:
        values = list(getattr(record, measurement_attr).values())
        for left, right in combinations(values, 2):
            diffs.append(abs(left - right))
    return diffs


def estimate_d3_threshold(records: list[EpochRecord], measurement_attr: str, quantile: float) -> float:
    diffs = iter_pairwise_abs_diffs(records, measurement_attr)
    if not diffs:
        return float("nan")
    return float(np.quantile(np.asarray(diffs, dtype=float), quantile))


def build_d3_frame(records: list[EpochRecord], measurement_attr: str, threshold: float, min_cluster_size: int):
    import pandas as pd
    ...
    for record in records:
        ...
        suspects = (
            detect_d3_spoofed_svids(dd_by_svid, threshold, min_cluster_size)
            if dd_by_svid and not np.isnan(threshold)
            else []
        )
        rows.append(
            {
                ...
                "suspects": ",".join(str(svid) for svid in suspects),
            }
        )
    return pd.DataFrame(rows, columns=["epoch_index", "tow_s", "num_visible_svids", "suspect_count", "suspects"])


def summarize_frame(frame, scenario_name: str, measurement: str, reference_svid: int, threshold: float):
    import pandas as pd
        ...
        [
            {
                ...
                "threshold": threshold,
                "any_suspect_fraction": float((frame["suspect_count"] > 0).mean()) if len(frame) else float("nan"),
                "mean_suspect_count": float(frame["suspect_count"].mean()) if len(frame) else float("nan"),
            }
        ]
    )


def save_plot(frame, scenario_name: str, measurement: str, output_dir: Path) -> None:
    import matplotlib
    ...
    ax.plot(
        ...
        frame["suspect_count"].to_numpy(dtype=float),
        color="tab:orange",
        label="D3 suspect count",
    )
    ax.set_xlabel("Matched epoch index")
    ax.set_ylabel("suspect satellites")
    ax.set_title(f"{scenario_name}: D3 on {measurement}")
    ax.grid(True)
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_dir / f"{scenario_name}_{measurement}_d3.png", dpi=200)
    plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone D3 detector batch runner.")
    ...
    parser.add_argument(
        "--measurement",
        type=str,
        choices=sorted(MEASUREMENT_ATTRS),
        default="smoothed_code",
        help="DD measurement family used by D3.",
    )
...
def main() -> None:
    ...
    measurement_attr = MEASUREMENT_ATTRS[config.measurement]
    ...
    threshold = estimate_d3_threshold(scenario_records["non_sp"], measurement_attr, config.clean_quantile)
    save_json(
        {
            ...
            "config": {
                ...
                "clean_quantile": config.clean_quantile,
                "min_cluster_size": config.min_cluster_size,
                "skip_initial_epochs": config.skip_initial_epochs,
                "max_epochs_per_file": config.max_epochs_per_file,
            },
        },
        output_root / "d3_threshold.json",
    )

    summary_frames = []
    for scenario_name, records in scenario_records.items():
        frame = build_d3_frame(records, measurement_attr, threshold, config.min_cluster_size)
        frame.to_csv(output_root / f"{scenario_name}_{config.measurement}_d3.csv", index=False)
        save_plot(frame, scenario_name, config.measurement, output_root / "plots")
        summary_frames.append(
            summarize_frame(frame, scenario_name, config.measurement, references[scenario_name], threshold)
        )

    summary_df = pd.concat(summary_frames, ignore_index=True)
    summary_df.to_csv(output_root / "d3_summary.csv", index=False)
