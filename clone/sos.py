#!/usr/bin/env python3
"""
Standalone Sum-of-Squares (SoS) batch detector for GNSS spoofing experiments.

This script reads UBX logs directly, builds the requested DD measurement family,
and evaluates the SoS detector without importing the shared comparison pipeline.
"""
...
def validate_config(config: SoSConfig) -> None:
...
def estimate_sos_threshold(records: list[EpochRecord], measurement_attr: str, quantile: float) -> float:
    values = [
        compute_sos(getattr(record, measurement_attr))
        for record in records
        if getattr(record, measurement_attr)
    ]
    if not values:
        return float("nan")
...
def build_sos_frame(records: list[EpochRecord], measurement_attr: str, threshold: float):
    import pandas as pd
        ...
        rows.append(
            {
                ...
                "sos": sos_value,
                "spoof": bool(sos_value < threshold) if not np.isnan(sos_value) and not np.isnan(threshold) else False,
            }
        )
    return pd.DataFrame(rows, columns=["epoch_index", "tow_s", "num_visible_svids", "sos", "spoof"])
        ...
        [
            {
                ...
                "threshold": threshold,
                "spoof_fraction": float(frame["spoof"].mean()) if len(frame) else float("nan"),
                "mean_sos": float(frame["sos"].mean()) if len(frame) else float("nan"),
            }
        ]
    )


def save_plot(frame, scenario_name: str, measurement: str, threshold: float, output_dir: Path) -> None:
    import matplotlib
    ...
    ax.plot(frame["epoch_index"].to_numpy(dtype=float), frame["sos"].to_numpy(dtype=float), label="SoS")
    if not np.isnan(threshold):
        ax.axhline(threshold, color="red", linestyle="--", label="clean-derived threshold")
    ax.set_xlabel("Matched epoch index")
    ax.set_ylabel("SoS")
    ax.set_title(f"{scenario_name}: SoS on {measurement}")
    ax.grid(True)
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_dir / f"{scenario_name}_{measurement}_sos.png", dpi=200)
    plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Standalone SoS detector batch runner.")
    ...
    parser.add_argument(
        ...
        help="DD measurement family used by SoS.",
    )
...
def main() -> None:
    ...
    threshold = estimate_sos_threshold(scenario_records["non_sp"], measurement_attr, config.clean_quantile)
    save_json(
        {
        ...
        output_root / "sos_threshold.json",
    )

    summary_frames = []
    for scenario_name, records in scenario_records.items():
        frame = build_sos_frame(records, measurement_attr, threshold)
        frame.to_csv(output_root / f"{scenario_name}_{config.measurement}_sos.csv", index=False)
        save_plot(frame, scenario_name, config.measurement, threshold, output_root / "plots")
        summary_frames.append(
            summarize_frame(frame, scenario_name, config.measurement, references[scenario_name], threshold)
        )

    summary_df = pd.concat(summary_frames, ignore_index=True)
    summary_df.to_csv(output_root / "sos_summary.csv", index=False)
